    ...
    def avail_pawn_moves(self, rw, cl, board = current_board):
        ...
        if self.color == 'w':
            ...
            if (rw-1 >= 0) and (board[rw-1, cl] == 0):
                ...
                # If first is open, and pawn hasn't moved, also checks 2nd spot
                if (rw-2 >= 0) and (board[rw-2, cl] == 0) and (self.moves == 0):
                    allowed_moves[rw-2, cl] = 1
        else:
            if (rw+1 <= 7) and (board[rw+1, cl] == 0):
                allowed_moves[rw+1, cl] = 1
                if (rw+2 <= 7) and (board[rw+2, cl] == 0) and (self.moves == 0):
                    allowed_moves[rw+2, cl] = 1
        return allowed_moves
